...
define(["N/record", "N/log", "N/file", "N/email", "N/search", "N/ui/serverWidget", "N/format", "N/https", "N/url", "N/xml", "N/render", "N/runtime"], function (record, log, file, email, search, serverWidget, format, https, url, xml, render, runtime) {
    function doPost(params) {
        ...
        if (obj_passwordByEmail.length > 0) {
            var hash = new jsSHA(obj_passwordByEmail[0].values.custentity446, "TEXT");
            var hashOutput = hash.getHash("SHA-256", "HEX");

            if (param_password === hashOutput) { // Validacion de login
                log.debug("CLAVE", "Acceso Correcto");

                // Search Customer by email
                var searchCreate_customerByEmail = search.create({
                    type: search.Type.CUSTOMER,
                    filters: [{
                        name: "email",
                        operator: "is",
                    ...
                    columns: [{
                        ...
                        /*
                        {
                            name: "custevent331" // PREGUNTAR A KARI
                        }, */
                        {
                            name: "custentity447" // linkPhoto
                        },
                        {
                            name: "custentity444" // level
                        },
                        {
                            name: "custentity303"
                        }
                ...
                var searchResult_customerByEmail = searchCreate_customerByEmail.run().getRange({
                    start: 0,
                    end: 1
                });
                var obj_customerByEmail = JSON.parse(JSON.stringify(searchResult_customerByEmail));
                ...
                var searchHistory = search.create({
                    ...
                    filters: [{
                        ...
                        join: 'customer',
                        operator: 'is',
                        values: obj_customerByEmail[0].values.internalid[0].value
                    }, {
                        name: 'title',
                        operator: 'startswith',
                        values: 'Histor'
                ...
                var fechaNacimiento = obj_case_historiaClinica.getValue({
                    fieldId: "custevent331"
                });

                //log.debug("Perfil", obj_customerByEmail[0].values.internalid[0].value);
                //log.debug("Imagen", obj_customerByEmail[0].values.custentity447[0].value);

                var imageCustomer = obj_customerByEmail[0].values.custentity447[0].value;
                var file_content_image;
                if (imageCustomer == "" || imageCustomer == null || imageCustomer == undefined) {
                    file_content_image = "";
                } else {
                    var fileObj_image = file.load({
                        id: imageCustomer
                    });
                ...
                var login = {
                    "level": obj_customerByEmail[0].values.custentity444[0].value,
                    "custevent331": fechaNacimiento,
                    "linkPhoto": file_content_image,
                    "firstname": obj_customerByEmail[0].values.firstname,
                    "lastname": obj_customerByEmail[0].values.lastname,
                    "custentity25": obj_customerByEmail[0].values.custentity25[0].text,
                    "sucursalID": obj_customerByEmail[0].values.custentity25[0].value,
                    "employment": obj_customerByEmail[0].values.custentity303,
                    "internalid": obj_customerByEmail[0].values.internalid[0].value
                }

                // Search Shedules by Customer id
                var searchCreate_shedluesByCustomer = search.create({
                    ...
                    filters: [{
                        ...
                        "operator": "equalto",
                        "values": [
                            obj_customerByEmail[0].values.internalid[0].value
                        ],
                        "isor": false,
                        "isnot": false,
                        "leftparens": 0,
                ...
                var searchResult_shedluesByCustomer = searchCreate_shedluesByCustomer.run().getRange({
                    start: 0,
                    end: 1000
                });
                var obj_shedluesByCustomer = JSON.parse(JSON.stringify(searchResult_shedluesByCustomer));

                log.debug("agendas", obj_shedluesByCustomer);

                var agendas = [];

                for (var sbc in obj_shedluesByCustomer) {
                    var statusText = (obj_shedluesByCustomer[sbc].values.custevent1.length > 0) ? obj_shedluesByCustomer[sbc].values.custevent1[0].text : ""
                    agendas.push({
                        "idEvento": obj_shedluesByCustomer[sbc].id,
                        "type": obj_shedluesByCustomer[sbc].values.custevent70[0].text,
                        "title": obj_shedluesByCustomer[sbc].values.title,
                        "date": obj_shedluesByCustomer[sbc].values.startdate,
                        "hour": obj_shedluesByCustomer[sbc].values.starttime,
                        "status": statusText,
                        "qualificationConsultor": obj_shedluesByCustomer[sbc].values.custevent481,
                        "qualificationEjecutivo": obj_shedluesByCustomer[sbc].values.custevent1210
                    });
                }

                var searchCreate_salesByCustomer = search.create({
                    ...
                    columns: [{
                        ...
                        {
                            "name": "custbody_especificaciones",
                            "label": "Notas",
                            "type": "clobtext",
                            "sortdir": "NONE"
                        },
                ...
                var pageData_searchResult_salesByCustomer = searchCreate_salesByCustomer.runPaged({
                    pageSize: 10
                });

                pageData_searchResult_salesByCustomer.pageRanges.forEach(function (pageRange) {
                    page = pageData_searchResult_salesByCustomer.fetch({
                ...
                for (var d = 0; d < preData.length; d++) {
                    ...
                    if (typeTransaction === "invoice") {
                        ...
                        var idInvoice = idTransaction;
                        /*recordLoad_TransactionInvoice.getValue({
                                       fieldId: "id"
                                     });*/
                        ...
                        var salesrep = recordLoad_TransactionInvoice.getText({
                            fieldId: "salesrep"
                        });
                ...
                var compras = [{
                    "products": [{
                        "idTransaction": "715600",
                        "item": "2105",
                        "itemText": "RINOPLASTIA",
                        "price": "60344.8275",
                        "quantity": "0"
                    }],
                    "typeTransaction": "invoice",
                    "dateInvoice": "2020-6-5",
                    "status": "Anulado",
                    "amountPaid": 0
                }];
                ...
                // ERROR DE ACCESO DE USUARIO POR PASS
                return {
                    "status": "ERROR_LOGIN"
                }
            }
        } else {
            // ERROR DE ACCESO DE USUARIO POR MAIL
            return {
                "status": "ERROR_LOGIN"
            }
        }
