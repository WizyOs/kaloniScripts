...
function pageInit(form, type) {
	...
	var frmname = nlapiGetFieldText('customform');

	// Factura
	if (frmname == 'OSS Factura Vta MX')
      {
	...
	// Nota de Cargo
	if (frmname == 'OSS Nota de Cargo') {
		nlapiSetFieldValue('custbody_tipodoc_cxc', 2);
		nlapiDisableField('custbody_tipodoc_cxc', true);
	}
...
function loadmails(){
	 ...
	 if (centity!=null && centity!=''){
		 var fieldname = [ 'email','subsidiary' ];
