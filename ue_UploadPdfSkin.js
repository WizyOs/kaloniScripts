...
function beforeLoadUploadPdfSkin(type, form)
{
  if(type == "view")
  {
    try
        ...
        if(hg.substring(0,3) == "HG-")
        {
            ...
            if(searchResult != null)
            {
              for (var i = 0 ; i < searchResult.length; i++)
              {
               ...
               nlapiLogExecution('ERROR', 'ue_UploadPdfSkin.js | beforeLoadUploadPdfSkin()', 'folderId _ var i position value: ' + folderId + ' _ ' + i);
              }
              if(folderId != null)
              {
                  var url = urlSkin;
                  ...
                  nlapiLogExecution('ERROR', 'ue_UploadPdfSkin.js | beforeLoadUploadPdfSkin()', 'local urlFile: ' + urlFile);
                  customerRecord.setFieldValue('custentity391', urlFile);
                  nlapiSubmitRecord(customerRecord, false, true);
              }
              else
              {
                nlapiLogExecution('ERROR', 'ue_UploadPdfSkin.js | beforeLoadUploadPdfSkin()', 'El folderId encontrado es nulo: ' + folderId);
              }
            }
            else
            {
               nlapiLogExecution('ERROR', 'ue_UploadPdfSkin.js | beforeLoadUploadPdfSkin()', 'No se encontró la carpeta llamada: ' + folderToFind);
            }
        }
      }
    }catch(e){
