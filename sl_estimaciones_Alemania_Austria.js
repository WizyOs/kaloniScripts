...
define(['N/record', 'N/email', 'N/file', 'N/log', 'N/redirect', 'N/render', 'N/xml', 'N/search'],
function(record, email, file, log, redirect, render, xmlref, search){
    ...
    function onRequest(context)
    {
    	try
        {
            var estimateId = context.request.parameters.estimateId;
            log.debug('estimateId: ', estimateId);
			if(estimateId != "" && estimateId != null)
            {
 			  var estimate = record.load({type:'estimate', id:estimateId, isDynamic:true});
              var tranid = estimate.getValue({fieldId: 'tranid'});
              var location = estimate.getValue({fieldId: 'location'});
              var locationText = estimate.getText({fieldId: 'location'});
              var subtotalField = estimate.getValue({fieldId: 'subtotal'});
              log.debug('subtotalField: ', subtotalField);
              var discounttotalField = estimate.getValue({fieldId: 'discounttotal'});
              log.debug('discounttotalField: ', discounttotalField);
              var taxtotalField = estimate.getValue({fieldId: 'taxtotal'});
              log.debug('taxtotalField: ', taxtotalField);
              var totalField = estimate.getValue({fieldId: 'total'});
              log.debug('totalField: ', totalField);
              var entity = estimate.getValue({fieldId: 'entity'}); // cliente
              var subsidiary = estimate.getValue({fieldId: 'subsidiary'});
              ...
              {
                var customerRec = record.load({type:'customer', id:entity, isDynamic:true});
                var altname = customerRec.getValue({fieldId: 'altname'});
                var defaultaddress = customerRec.getValue({fieldId: 'defaultaddress'});
                var entityid = customerRec.getValue({fieldId: 'entityid'}); // HG
                ...
                {
                   dataCompanyPDF = "<table style=\"width:100%; font-family:'Aria', sans-serif\">";
                   dataCompanyPDF += '<tr><td align="left" color="black" font-size="10px" width="50%"><u>Kaloni GmbH - Sonnenstr. 31 - 85609 Hamburgo</u></td> <td align="right" color="black" width="50%"><b>Kaloni GmbH</b></td></tr>';
                   dataCompanyPDF += '<tr><td align="left" color="black" width="50%">'+altname+'</td> <td align="right" width="50%">Sonnenstraße 31</td></tr>';
                   dataCompanyPDF += '<tr><td align="left" color="black" width="50%">'+defaultaddress+'</td> <td align="right" width="50%">85609 Hamburgo</td></tr></table>';
                ...
                {
                  var fileObjImg = file.load({id:'5813680'});
                  var imgURLForPDF = fileObjImg.url;
                  var imageUrl = xmlref.escape({xmlText:'https://3559763.app.netsuite.com'+ imgURLForPDF}); //https://efactory-kaloni.com/SENDWA/images/kaloni_Logo.png
                  ...
                  xml += "</macrolist>";
                  xml += "</head>";
                  //background=\"https://efactory-kaloni.com/SENDWA/images/membreteSN.jpg\"
				  ...
				  xml += dataCompanyPDF;
                  xml += '<p></p>';
                  xml += "<table style=\"width:100%; font-family:'Aria', sans-serif\"><tr><td font-size=\"20px\"><b>Angebot</b></td><td align=\"right\">Angebotsdatum: "+fecha+"</td></tr><tr><td> </td><td align=\"right\">Kundennummer: "+entityid+"</td></tr><tr><td><b>Angebot Nr. "+tranid+"</b></td><td> </td></tr></table>";
                  ...
                  var item_Lines = estimate.getLineCount({sublistId : 'item'});
                  if(item_Lines > 0)
                  {
                    for(var i = 0; i < item_Lines; i++)
                    {
                      ...
                      var valor_quantity = estimate.getSublistValue({sublistId:'item', fieldId:'quantity', line:i}); // cantidad
                      ...
                      var valor_amount = estimate.getSublistValue({sublistId:'item', fieldId:'amount', line:i}); // precio unitario item
                      //if(parseFloat(valor_amount) > 0){log.debug('valor_amount: ', parseFloat(valor_amount));}
                      //log.debug('isNaN(parseFloat(valor_amount)): ', isNaN(parseFloat(valor_amount)));

                      var valor_description = estimate.getSublistText({sublistId:'item', fieldId:'description', line:i}); //descripcion
                      //if(valor_description != "" && valor_description != null){log.debug('valor_description: ', valor_description);}

                      var valor_tax1amt = estimate.getSublistValue({sublistId:'item', fieldId:'tax1amt', line:i}); // impuesto aplicado
                      ...
                      var valor_grossamt = estimate.getSublistValue({sublistId:'item', fieldId:'grossamt', line:i}); // (precio total item + impuesto aplicado)
                      ...
                      {
                        log.debug('Error 1: ', 'Falta algún dato necesario para hacer la Estimación!!');
                        redirect.toRecord({type : 'estimate', id : estimateId});
                        return;
                        break;
                      }
                    }
                  ...
                  {
                    log.debug('Error 2: ', 'Falta algún dato necesario para hacer la Estimación!!');
                    redirect.toRecord({type : 'estimate', id : estimateId});
                    return;
                  }

                  xml += '<p></p>';
                  xml += "<table style=\"width:100%;\"><tr><td color=\"black\">Zahlungsbedingungen</td></tr> <tr><td color=\"black\">Dieser Betrag muss spätestens 10 Tage vor dem geplanten OP-Tag auf dem Konto von</td></tr> <tr><td color=\"black\">Kaloni einbezahlt worden sein.</td></tr></table>";
                  ...
                  if(new_nc_pdf_id != "" && new_nc_pdf_id != null)
                  {
                    ...
                    if(update_txt_folios_id != "" && update_txt_folios_id != null)
                    {
                      ...
                      estimate.setValue({fieldId: 'custbody156', value: ncf});
                      estimate.setValue({fieldId: 'custbody159', value: pdf_generated.url});
                      estimate.setValue({fieldId: 'custbody161', value: pdf_generated.getContents()});
                      estimate.save({enableSourcing: false, ignoreMandatoryFields: true});
                      redirect.toRecord({type:'estimate', id:estimateId});
                      //return;
                    }
                  }
                ...
                {
                  log.debug('Error: ', 'Faltan datos para generar la Estimación!!');
                  redirect.toRecord({type : 'estimate', id : estimateId});
                  return;
                }
              }
