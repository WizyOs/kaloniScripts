...
define(['N/record', 'N/file', 'N/log', 'N/ui/serverWidget'], function(record, file, log, serverWidget) {
   ...
   function beforeLoad(context)
   {
     ...
     {
        var record_estimate = context.newRecord.id;
    	var objRecord = record.load({type: 'estimate', id: record_estimate, isDynamic: true});
        ...
        if(subsidiary == "13" || subsidiary == "17") // generated_pdf_data == "" || generated_pdf_data == null
        {
            ...
            html += '  document.getElementById("tbl_secondaryprint").style.display = "none";';
            html += '  document.getElementById("tbl_print").style.display = "none";';
            html += '  jQuery("#tbl_menu_printinlocale, #tbl_menu_printinlocale").hide();';
            html += '}';
            html += '</script>';
            ...
            context.form.addButton({id: "custpage_estimate_alemania", label: "Print Estimate", functionName: "onButtonClick_estimateAlemania"});
          	context.form.clientScriptModulePath = "SuiteScripts/cs_estimaciones_Alemania_Austria.js";
        }
        /*else
        {
