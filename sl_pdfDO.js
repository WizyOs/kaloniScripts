 ...
 */
	// *********************************************************************************************************
function crearPDFinvoiceSTOdo(valid){
  	try
    {
    //Invoice
    ...
    var folio = "";
     ...
     xml += '<tr><td align=\"left\" color=\"black\"><b>FECHA: '+ dateToday +' </b></td></tr></table>';
                ...
                for (var i = 1; i <= conVal; i++)
                {
                  ...
                  xml += '<tr><td align=\"center\">' + currentQuantity + ' x ' + currentRate + '</td>';
    		    ...
    		    var urlFile = newfilePDF.getURL();
              	...
              	nlapiSubmitRecord(inRecord, false, true);
      		 	...
      		 	nlapiLogExecution('ERROR', 'function crearPDFinvoiceSTOdo() ', 'Value PDF id: ' + newfileId);
              	return true;
    }
  	catch(error)
    {
      var msgError = error;
      nlapiLogExecution('ERROR', 'Error al crear PDF STO DO', msgError);
      return false;
    }
}
...
function gererarPDF(request, response)
{
  try
  {
	var id = request.getParameter('poid');
  	nlapiLogExecution('ERROR', 'PDF DO: ', 'Value request id parameter: ' + id);

	  var result = crearPDFinvoiceSTOdo(id);
      if(result === true) // || (result !== false && result !== null)
      {
        nlapiLogExecution('ERROR', 'function gererarPDF(): ', 'Factura STO DO creada correctamente!! Invoice record id: ' + id);
        nlapiLogExecution('ERROR', 'function gererarPDF() ', 'Submitted the STO Invoice now redirecting...');
      ...
      {
        nlapiLogExecution('ERROR', ' function gererarPDF(): ', 'Factura STO DO no creada!! Invoice record id: ' + id);
        nlapiLogExecution('ERROR', 'function gererarPDF() ', 'Submitted the STO Invoice now redirecting...');
        nlapiSetRedirectURL('RECORD', 'invoice', id, false);
      }
  }
  ...
  {
    var msgError = err2;
	nlapiLogExecution('ERROR', 'Error function gererarPDF()', msgError);
  }
}
